package com.tiagomaniero.pokedex.controller;

import com.tiagomaniero.pokedex.model.Pokemon;
import com.tiagomaniero.pokedex.model.PokemonEvent;
import com.tiagomaniero.pokedex.repository.PokemonRepository;
...
public class PokemonController {
    ...
    @GetMapping("/id/{id}")
    public Mono<ResponseEntity<Pokemon>> getPokemonById(@PathVariable(value = "id") String id){
    ...
    @DeleteMapping("/id/{id}")
    public Mono<ResponseEntity<Object>> deletePokemonById(@PathVariable(value = "id") String id){
        return repository.findById(id)
                .flatMap( existingPokemon ->
    ...
    public Flux<PokemonEvent> getPokemonEvents(){
        return Flux.interval(Duration.ofSeconds(5))
                .map(valor -> new PokemonEvent(valor, "pokemons"));
    }
}
